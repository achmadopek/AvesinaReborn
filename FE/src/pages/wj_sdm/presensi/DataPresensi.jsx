  ...
  return (
    <div className="card shadow-sm card-theme mt-3">
      <div className="card-header py-2 px-3">
        <h6 className="mb-0">Data Presensi Pegawai</h6>
      </div>
          ...
          <table className="table table-theme table-sm table-bordered">
            <thead>
              <tr>
                <th>No</th>
                <th>Nama Pegawai</th>
                {!isMobile && <th>Periode</th>}
                <th>Alpha (%)</th>
                {!isMobile && <th>Nilai Potongan</th>}
                <th>Ver-Val</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <span
                        ...
                        onClick={() => handleSelectPresensi(item, index)}
                      >
                        {item.nama_pegawai}
                      </span>
                    </td>
                    {!isMobile && <td>{formatDate(item.periode)}</td>}
                    <td>{item.prosentase_alpha} %</td>
                    {!isMobile && <td>Rp {item.nilai.toLocaleString()}</td>}
                    <td>
                      {item.verified_by ? "✅" : "❌"} -{" "}
                      {item.validated_by ? "✅" : "❌"}
                    </td>

  ...
  return (
    <div className="card shadow-sm card-theme mt-3">
      <div className="card-header py-2 px-3">
        <h6 className="mb-0">Data Komponen Penghasilan</h6>
      </div>
              ...
              {Array.isArray(data) && data.length > 0 ? (
                data.map((item, index) => (
                  <tr key={item.penghasilan_code || index}>
                    ...
                    <td>{formatNumber(item.default_nilai)}</td>
                    <td>{item.jenis}</td>
                  </tr>
                ))
              ) : (
...
export default DataKomponenPenghasilan;

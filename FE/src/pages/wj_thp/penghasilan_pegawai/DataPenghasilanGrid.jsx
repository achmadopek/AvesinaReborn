...
import { unpivotPenghasilanData } from "../../../utils/pivotPenghasilan";

import LoadingOverlay from "../../../components/LoadingOverlay";

export default function DataPenghasilanGrid({
  data,
  komponenMapping,
  onSave,
  onEndroll,
  ...
  const handleSaveClick = () => {
    const unpivoted = unpivotPenghasilanData(rows, komponenMapping);
    onSave(unpivoted);
  };
            ...
            <tr key={row.peg_id}>
              ...
              <td>
                ...
                <span style={{ fontSize: "8pt" }}>
                  ({row.durasi} hari kalender)
                </span>
              </td>
              {komponenMapping.map((k) => {
                const komponen = row.komponen?.find(
                  (c) => c.penghasilan_code == k.code
                );

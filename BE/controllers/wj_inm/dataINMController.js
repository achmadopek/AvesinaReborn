...
exports.getINMHarianByUnit = (req, res) => {
  ...
  SELECT
    ...
    d.is_meet_standard
  FROM inm_harian h
  JOIN inm_harian_detail d ON d.harian_id = h.id
  WHERE h.unit_id = ?
    AND h.tgl_input = ?
`;
...
exports.getRekapBulanan = (req, res) => {
  ...
  const sql = `
    ...
    -- data harian
    LEFT JOIN inm_harian h
      ON h.tgl_input = DATE(d.tgl)
      AND h.unit_id = u.id

    LEFT JOIN inm_harian_detail sd
...
exports.generateRekapValidasi = (req, res) => {
  ...
  db_lokal.query(sqlUnit, [instalasi_id], (err, units) => {
    ...
    const sqlInsertRekap = `
      INSERT INTO inm_rekap (instalasi_id, bulan, tahun)
      VALUES (?, ?, ?)

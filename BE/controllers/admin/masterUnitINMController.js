...
exports.getData = (req, res) => {
  ...
  const dataQuery = `
    ...
    LEFT JOIN inm_bidang b ON b.id = u.bidang_id
    LEFT JOIN inm_instalasi i ON i.id = u.instalasi_id
    LEFT JOIN inm_unit_group_pelayanan ug ON ug.unit_id = u.id
    LEFT JOIN inm_group_pelayanan g ON g.id = ug.group_pelayanan_id
...
exports.createUnit = (req, res) => {
  ...
  db.getConnection((err, conn) => {
    ...
    conn.beginTransaction((err) => {
      ...
      // 1. insert inm_unit (TANPA group_pelayanan_id)
      const sqlUnit = `
        INSERT INTO inm_unit (
          instalasi_id,
          bidang_id,
          srvc_unit_id,
          nama_unit,
      ...
      conn.query(
        ...
        (err, result) => {
          ...
          const sqlRelasi = `
          INSERT INTO inm_unit_group_pelayanan (unit_id, group_pelayanan_id)
          VALUES ?
        `;
          ...
          conn.query(sqlRelasi, [values], (err) => {
            if (err) {
              return conn.rollback(() => {
                ...
                res
                  .status(500)
                  .json({ message: "Gagal insert relasi group pelayanan" });
              });
            }
    ...
    conn.beginTransaction((err) => {
      ...
      conn.query(
        ...
        (err) => {
          ...
          DELETE FROM inm_unit_group_pelayanan
          WHERE unit_id = ?
        `;
          ...
          conn.query(sqlDeleteRelasi, [id], (err) => {
            ...
            if (!group_pelayanan_id || group_pelayanan_id.length === 0) {
              return conn.commit((err) => {
                ...
                return res.json({
                  message: "Unit berhasil diupdate (tanpa group)",
                });
              });
            }

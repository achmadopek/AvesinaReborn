...
exports.getRekapIndikator = async (req, res) => {
  try {
    ...
    if (mode === "instalasi") {
      ...
      joinEntity = "LEFT JOIN spm_instalasi ins ON ins.id = u.instalasi_id";
      selectEntity = "ins.nama_instalasi AS nama_entity";
    }
    if (mode === "bidang") {
      whereUnit = "u.bidang_id = ?";
      joinEntity = "LEFT JOIN spm_bidang b ON b.id = u.bidang_id";
    ...
    const sql = `
      ...
      JOIN spm_harian_detail sd ON sd.indikator_id = i.id
      JOIN spm_harian h ON h.id = sd.harian_id
      JOIN spm_unit u ON u.id = h.unit_id
      ${joinEntity}

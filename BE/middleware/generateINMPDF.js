...
const generateINMPDF = async (
  ...
  const [rows] = await db.promise().query(
    `
        SELECT 
            ...
            FROM inm_harian_detail d

            JOIN inm_indikator ind 
  ...
  return new Promise(async (resolve, reject) => {
    const fileName = `inm_${harian.id}.pdf`;
    const filePath = path.join(__dirname, "../uploads/inm", fileName);
    ...
    doc.fontSize(14).text("UOBK RSUD WALUYO JATI KAB. PROBOLINGGO", 0, 40, {
      align: "center",
    });

    doc.fontSize(12).text("LAPORAN INM HARIAN UNIT", {
      align: "center",
    });
    ...
    Object.values(grouped).forEach((group, gIndex) => {
      ...
      group.items.forEach((r, i) => {
        ...
        doc
          .fontSize(9)
          .text(r.is_meet_standard ? "Memenuhi" : "Belum", colStat, y);
    ...
    const qrText = `${process.env.BASE_URL}/api/inm/EntriINMHarian/download/${harian.id}`;
    ...
    doc
      .fontSize(8)
      .text(
        "Dokumen ini dihasilkan otomatis oleh Sistem INM Rumah Sakit",
        0,
        footerY,
        { align: "center" },
      );
